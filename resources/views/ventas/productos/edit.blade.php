...
@php
  ...
  $existingPrecios = $producto['precios'] ?? [];
@endphp

<form method="POST" action="{{ route('productos.update', $producto['id']) }}" class="max-w-2xl mx-auto space-y-5">
  @csrf
  @method('PUT')
  <div class="flex items-center justify-between">
    <h1 class="text-2xl font-bold text-slate-900">Editar Producto / Servicio</h1>
    <div class="flex gap-2">
      <a href="{{ route('productos.index') }}" class="px-4 py-2 rounded-full border text-sm text-slate-600 hover:bg-slate-50">Cancelar</a>
      <button type="submit" class="px-4 py-2 rounded-full bg-[#ecfe88] text-slate-900 text-sm font-bold shadow-sm hover:bg-[#d9ef60]">Actualizar</button>
    </div>
  </div>
  ...
  <div class="bg-white rounded-2xl shadow-soft border p-6 space-y-5">
    ...
    <div>
      ...
      <input type="text" name="nombre" value="{{ $producto['nombre'] }}" required placeholder="Ej. Diseño Web Corporativo"
    ...
    <div>
      ...
      <textarea name="descripcion" rows="6" placeholder="Detalles del producto o servicio..."
        class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-slate-900 focus:outline-none focus:ring-2 focus:ring-lime-300 focus:border-lime-300 transition-shadow resize-none">{{ $producto['descripcion'] ?? '' }}</textarea>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        ...
        <div class="relative">
          ...
          <input type="number" name="precio" value="{{ $producto['precio'] }}" required step="0.01" min="0" placeholder="0.00"
    ...
    <div class="grid grid-cols-2 gap-4">
      <div>
        ...
        <input type="text" name="sku" value="{{ $producto['sku'] ?? '' }}" placeholder="Ej. WEB-001"
          class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-slate-900 focus:outline-none focus:ring-2 focus:ring-lime-300 focus:border-lime-300 transition-shadow">
      </div>
      <div>
        <label class="block text-sm font-medium text-slate-600 mb-1">Stock <span class="text-slate-400 font-normal">(Opcional)</span></label>
        <input type="number" name="stock" value="{{ $producto['stock'] ?? '' }}" placeholder="Solo para productos físicos"
          class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-slate-900 focus:outline-none focus:ring-2 focus:ring-lime-300 focus:border-lime-300 transition-shadow">
      </div>
    </div>
    ...
    @php
      $serviceReminderEnabled = (bool) ($producto['service_expiry_reminder_enabled'] ?? false);
      $serviceReminderDays = (int) ($producto['service_expiry_reminder_days_before'] ?? 7);
      if ($serviceReminderDays < 1) $serviceReminderDays = 7;
    @endphp
    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
      <label class="inline-flex items-center gap-2 cursor-pointer select-none">
        <input type="checkbox" name="service_expiry_reminder_enabled" id="serviceReminderEnabled" value="1"
          {{ $serviceReminderEnabled ? 'checked' : '' }}
      ...
      <div id="serviceReminderDaysWrap" class="mt-3">
        ...
        <input type="number" name="service_expiry_reminder_days_before" id="serviceReminderDays" min="1" max="90" value="{{ $serviceReminderDays }}"
  ...
  <div class="bg-white rounded-2xl shadow-soft border p-6">
    ...
    <div id="preciosExtras" class="space-y-3">
      @foreach($existingPrecios as $cur => $val)
        @if($val !== null && $val !== '' && strtoupper($cur) !== $base)
        <div class="flex items-center gap-3 currency-row">
          <select name="precios[{{ strtoupper($cur) }}]"
            ...
            @foreach($otherCurrencies as $c)
              <option value="{{ $c }}" {{ strtoupper($cur) === $c ? 'selected' : '' }}>{{ $c }}</option>
            @endforeach
          </select>
          <div class="relative flex-1">
            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">$</span>
            <input type="number" name="precios[{{ strtoupper($cur) }}]" value="{{ $val }}" step="0.01" min="0" placeholder="0.00"
    ...
    <p id="noCurrenciesMsg" class="text-xs text-slate-400 text-center py-2" style="{{ count(array_filter($existingPrecios, fn($v) => $v !== null && $v !== '')) ? 'display:none' : '' }}">No hay precios adicionales</p>
  </div>
</form>
...
(function(){
  ...
  container.querySelectorAll('.currency-row').forEach(row => attachRowEvents(row));
  ...
  document.getElementById('btnAddCurrency').addEventListener('click', () => {
    const usedCurrencies = [...container.querySelectorAll('.currency-select')].map(s => s.value);
    const available = currencies.filter(c => !usedCurrencies.includes(c));
    if (!available.length) return;

    const row = document.createElement('div');
    row.className = 'flex items-center gap-3 currency-row';
    row.innerHTML = `
      <select name="precios[${available[0]}]"
        class="w-28 rounded-xl border border-slate-200 px-3 py-2 bg-white text-sm font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-lime-300 focus:border-lime-300 currency-select">

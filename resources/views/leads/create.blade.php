...
@section('content')
  <div class="max-w-2xl bg-white rounded-2xl shadow border p-6">
    <div class="text-xl font-bold mb-4">Nuevo lead</div>
    <form method="POST" action="{{ route('leads.store') }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
      @csrf
      <div>
        <label class="text-sm font-medium">Nombre</label>
        <input name="nombre" class="w-full rounded-lg border p-2.5" required>
      </div>
      <div>
        <label class="text-sm font-medium">Email</label>
        <input name="email" type="email" class="w-full rounded-lg border p-2.5">
      </div>
      <div>
        <label class="text-sm font-medium">Teléfono</label>
        <input name="telefono" class="w-full rounded-lg border p-2.5">
      </div>
      <div>
      ...
      <div>
        ...
        <input name="presupuesto_estimado" type="number" step="0.01" class="w-full rounded-lg border p-2.5" placeholder="Ej. 2500000">
      </div>
      <div>
        <label class="text-sm font-medium">Origen</label>
        <input name="origen" class="w-full rounded-lg border p-2.5">
      </div>
      <div class="md:col-span-2">
        <label class="text-sm font-medium">Encargados</label>
        @php $oldAssigned = collect(old('encargados', []))->map(fn($v) => (string) $v)->filter()->values()->all(); @endphp
      ...
      <div class="md:col-span-2">
        ...
        <textarea name="notas" class="w-full rounded-lg border p-2.5" rows="3"></textarea>
      </div>
      <div>
        <label class="text-sm font-medium">Recordatorio</label>
        <div class="relative">
          <input name="recordatorio" id="recordatorio" type="text" class="w-full rounded-lg border p-2.5 bg-white cursor-pointer" placeholder="Seleccionar fecha y hora">
        </div>
      </div>
...
<script>
  function initAssigneePicker(users, initialSelected) {
    ...
    initAssigneePicker(@json($assignableUsers ?? []), @json($oldAssigned));

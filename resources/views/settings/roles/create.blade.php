...
<div class="max-w-2xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-2xl font-bold text-slate-900">Crear Nuevo Rol</h2>
    ...
    <form action="{{ route('settings.roles.store') }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8 space-y-6">
        @csrf

        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Nombre del Rol</label>
            <input type="text" name="name" class="block w-full px-3 py-2.5 border border-slate-300 rounded-xl focus:ring-[#ecfe88] focus:border-[#ecfe88] sm:text-sm" placeholder="Ej. Supervisor de Ventas" required>
        </div>

        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Descripción</label>
            <textarea name="description" rows="2" class="block w-full px-3 py-2.5 border border-slate-300 rounded-xl focus:ring-[#ecfe88] focus:border-[#ecfe88] sm:text-sm" placeholder="Breve descripción de las responsabilidades..."></textarea>
        </div>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                @foreach($module['perms'] as $perm => $permLabel)
                                    <label class="inline-flex items-center gap-2 cursor-pointer">
                                        <input type="checkbox" name="permissions[]" value="{{ $module['key'] }}.{{ $perm }}" class="perm-{{ $module['key'] }} form-checkbox text-lime-600 rounded border-gray-300 focus:ring-lime-500">
                                    ...
                                    <div class="rounded-xl border border-slate-200 bg-white p-3">
                                        ...
                                        <div class="grid grid-cols-2 gap-2">
                                            @foreach($child['perms'] as $perm => $permLabel)
                                                <label class="inline-flex items-center gap-2 cursor-pointer">
                                                    <input type="checkbox" name="permissions[]" value="{{ $child['key'] }}.{{ $perm }}" class="perm-{{ $child['key'] }} perm-ventas form-checkbox text-lime-600 rounded border-gray-300 focus:ring-lime-500">
                                                    <span class="text-sm text-slate-600">{{ $permLabel }}</span>
                                                </label>
                                            @endforeach
                                        </div>
                ...
                <div class="border-2 border-lime-200 rounded-xl p-4 bg-lime-50/40">
                    ...
                    <div class="space-y-2">
                        <label class="flex items-start gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5 cursor-pointer hover:bg-slate-50">
                            <input type="checkbox" name="permissions[]" value="timer.proyectos" class="perm-timer mt-0.5 form-checkbox text-lime-600 rounded border-gray-300 focus:ring-lime-500">
                            <div>
                        ...
                        <label class="flex items-start gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5 cursor-pointer hover:bg-slate-50">
                            <input type="checkbox" name="permissions[]" value="timer.leads" class="perm-timer mt-0.5 form-checkbox text-lime-600 rounded border-gray-300 focus:ring-lime-500">
                            <div>
        ...
        <div class="border-2 border-lime-200 rounded-xl p-4 bg-lime-50/40">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <label class="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 cursor-pointer hover:bg-slate-50">
                    <input type="checkbox" name="dashboard_tabs[]" value="resumen" checked class="form-checkbox text-lime-600 rounded border-gray-300 focus:ring-lime-500">
                    <span class="text-sm text-slate-700 font-semibold">Resumen</span>
                </label>
                <label class="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 cursor-pointer hover:bg-slate-50">
                    <input type="checkbox" name="dashboard_tabs[]" value="proyectos" checked class="form-checkbox text-lime-600 rounded border-gray-300 focus:ring-lime-500">
                    <span class="text-sm text-slate-700 font-semibold">Proyectos</span>
                </label>
                <label class="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 cursor-pointer hover:bg-slate-50">
                    <input type="checkbox" name="dashboard_tabs[]" value="ventas" checked class="form-checkbox text-lime-600 rounded border-gray-300 focus:ring-lime-500">
